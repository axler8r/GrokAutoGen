...
async def _lookup_investment_profile(client_name: str) -> str:
    try:
...
async def _verbose_run(task) -> None:
...
async def _quiet_run(task) -> None:
...
if __name__ == "__main__":
    ...
    if "--verbose" in sys.argv:
        ...
        asyncio.run(_verbose_run(task))
    else:
        asyncio.run(_quiet_run(task))
